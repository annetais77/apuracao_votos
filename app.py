...
def extrair_votos(texto):
    """Extrai @menções de comentários"""
    return [str(v).lower().strip().replace(" ", "") for v in re.findall(r'@[A-Za-z0-9_.-]+', str(texto))]

def listar_cidades():
    """Chama a função RPC do banco para pegar nomes únicos sem limite de linhas"""
    try:
        # Chamamos a função SQL que você criou no painel do Supabase
        res = supabase.rpc("obter_cidades_unicas").execute()
        if res.data:
            # O retorno vem como [{'nome_cidade': 'Bezerros'}, ...]
...
if modo == "⚙️ Painel ADM":
    ...
    if senha == "suasenha123":
        ...
        with t3:
            st.subheader("Cidades encontradas via RPC:")
            cidades_reais = listar_cidades()
            if cidades_reais:
                for c in cidades_reais:
                    st.write(f"📍 {c}")
            else:
                st.info("Nenhuma cidade encontrada no banco.")
    else:
...
# --- MODO PÚBLICO ---
    ...
    if not cidades_finais:
        ...
        if escolha != "-- Selecione --":
            # Aqui buscamos os dados da cidade (Limitado a 5000 linhas por cidade)
